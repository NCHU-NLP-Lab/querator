...
function EComponent(props) {
    let { q, k1, k2, dispatch, appState, radioOnSelect = false, onClick: onClickEvent } = props
    ...
    return (
        <div>
            ...
            >{editable ? (<span className="svg-big btn btn-outline-success">Confirm<MdCheck /></span>) : (<MdModeEdit />)}
            </button>
            {!editable ? ('') : (<span className="svg-big btn btn-outline-danger" onClick={() => {
                // setLastClick('cancel')
                setQText(qTextRollBack)
                setTimeout(() => {
                    setEditable(true)
